...
I came across [this challenge](https://dreamhack.io/wargame/challenges/2600) on Dreamhack, which served as the main inspiration for `cjail-revenge`.   

For contestants reading this and wondering why there wasn't a `cjail` chall, we lowkey forgot to add it in quals :P  
...
The flag is added in the same directory as the program source.  
...
RUN GCC_LIBDIR="$(dirname "$(gcc -print-libgcc-file-name)")" && \
    ln -sf /usr/bin/ld "$GCC_LIBDIR/ld"
...
`subprocess.run()` is called with the `capture_output` flag, so we can't output the flag through a stderr leak like `#include "./flag.txt"`.  
